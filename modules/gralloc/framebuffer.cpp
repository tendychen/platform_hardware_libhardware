...
static void fb_setOrientation(struct framebuffer_device_t* dev, int orientation)
{
    ...
    LOGD("%s setOrientation: orientation=%d\n", __FUNCTION__, orientation);
        ...
        switch (orientation) {
         ...
         case 3:
            ...
            if (ioctl(m->framebuffer->fd, FBIOPUT_VSCREENINFO, &m->rot_info) == -1) {
                LOGE("setOrientation: FBIOPUT_VSCREENINFO failed, errno=%d\n", errno);
            }

            if (ioctl(m->framebuffer->fd, FBIOGET_FSCREENINFO, &m->rot_finfo) == -1) {
                LOGE("setOrientation: FBIOGET_FSCREENINFO failed, errno=%d\n", errno);
            }
            else
                LOGE("setOrientation: new line_length=%d\n", m->rot_finfo.line_length);
            break;
         case 0:
         default:
            if (ioctl(m->framebuffer->fd, FBIOPUT_VSCREENINFO, &m->info) == -1) {
